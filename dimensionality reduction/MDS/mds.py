...
class Mds(object):
    ...
    def calcu_B(self):
        ...
        def dist_ij2(self, i, j):
            return np.sqrt(np.sum((self.D[i] - self.D[j])**2))
        ...
        for i, j in range(1, self.m + 1):
            ...
            d_j2 = dist_j2(j)
            d__2 = dist__2()
            self.B[i][j] = -0.5 * (d_ij2(i, j) - d_i2 - d_j2 + d__2)
        return self.B

    def eigen_decomp(self):
        e_vals, e_vecs = LA.eig(self.B)
        diag_mat_ori = np.diag(tuple(e_vals))
        diag_mat_d_ = diag_mat_ori[: int(self.d_), :int(self.d_)]
        eigen_vec_mat = e_vecs[:, :self.d_]
